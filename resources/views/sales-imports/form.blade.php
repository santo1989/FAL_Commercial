...
<div class="form-row">
    ...
    <div class="form-group col-md-4">
        <label for="accessories_qty">Accessories Qty (KG)</label>
        <input type="number" step="0.01" name="accessories_qty" id="accessories_qty"
            class="form-control @error('accessories_qty') is-invalid @enderror"
            value="{{ old('accessories_qty', $import->accessories_qty ?? 0) }}">
        @error('accessories_qty')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>

    <div class="form-group col-md-4">
        <label for="print_emb_qty">Print/Emb Qty (KG)</label>
        <input type="number" step="0.01" name="print_emb_qty" id="print_emb_qty"
            class="form-control @error('print_emb_qty') is-invalid @enderror"
            value="{{ old('print_emb_qty', $import->print_emb_qty ?? 0) }}">
        @error('print_emb_qty')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
    <div class="form-group col-md-4">
        <label for="print_emb_value">Print/Emb Value</label>
        <input type="number" step="0.01" name="print_emb_value" id="print_emb_value"
            class="form-control @error('print_emb_value') is-invalid @enderror"
            value="{{ old('print_emb_value', $import->print_emb_value ?? 0) }}">
        @error('print_emb_value')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
